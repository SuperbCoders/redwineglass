...
//        }
    ...
    func changeMonth(){
        self.currentDaySelector.isHidden = true
        setupViews()
    }
    ...
    private func setupHeader(){
        ...
        for i in 0..<7 {
            ...
            lab.text = symbols[i].localized().uppercased()
...
//            v.layer.borderWidth = 0.5
            ...
            if items.count >= 2 {
                lab.backgroundColor = Colors.selectionBGColor
                lab.textColor = .white
                lab.text = "\(items.count)"
                lab.font = UIFont(name: "SFProText-Bold", size: 10)
                lab.layer.cornerRadius = lab.bounds.width / 2
                lab.clipsToBounds = true
                if WineRecordItem.isSameOfItems(itms: items) {
                    if dat.isEqual(date: selectedDay) {
                        img.image = items[0].wineType.selectedImageBG
                    }else{
                        img.image = items[0].wineType.imageBG
                    }
                }else{
                    if dat.isEqual(date: selectedDay) {
                        img.image = WineType.selectedDoubleGlassImg
                    }else{
                        img.image = WineType.doubleGlassImg
                    }
                }
            }
            ...
            if dat.isEqual(date: selectedDay) {
                self.currentDaySelector.isHidden = false
                delay(0.2) {
                    self.layer.insertSublayer(self.currentDaySelector, at: 0)
                }
                currentDaySelector.frame = CGRect(x: 0, y: 0, width: h, height: h)
                self.currentDaySelector.position = v.center

                currentDaySelector.backgroundColor = Colors.selectionBGColor.cgColor
                currentDaySelector.cornerRadius = h / 2
            }
...
//            v.layer.borderWidth = 0.5
    ...
    @objc private func didTap(_ gest:UITapGestureRecognizer){
        if let view = gest.view{
            ...
            self.currentDaySelector.position = view.center
